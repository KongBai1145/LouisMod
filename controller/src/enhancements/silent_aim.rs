...
impl Enhancement for SilentAim {
    fn update(&mut self, ctx: &UpdateContext) -> anyhow::Result<()> {
        ...
        self.toggle
            .update(&mode, ctx.input, &settings.aim_silent_hotkey);
        ...
        let local_eye_pos = local_origin
            + Vector3::new(
                view_offset.m_vecX()?,
                view_offset.m_vecY()?,
                view_offset.m_vecZ()?,
        ...
        for entity_identity in entities.entities() {
            let class_name = match class_name_cache.lookup(&entity_identity.entity_class_info()?) {
                Ok(Some(name)) => name,
                _ => continue,
            };
            ...
            // Dormant check
            let scene_node = match pawn.m_pGameSceneNode()?.value_reference(memory.view_arc()) {
        ...
        if let Some((pitch, yaw)) = best_angle {
            let eye_angles_offset =
                runtime_offset!(15824, "client.dll", "C_CSPlayerPawn", "m_angEyeAngles");
            ...
            ctx.cs2
                .write_sized::<[f32; 4]>(write_addr, &original_angles)?;
        }
